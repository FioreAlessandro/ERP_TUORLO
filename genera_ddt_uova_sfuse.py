...
file_clienti = os.path.join(CONFIG, "CLIENTI_UOVA_SFUSE.xlsx")

df = pd.read_excel(file_clienti)

df.columns = df.columns.str.strip()

nome_input = cliente.lower().strip()

df["Nome_clean"] = df["Nome"].astype(str).str.lower().str.strip()

row = df[df["Nome_clean"] == nome_input]
email_cliente = ""

if not row.empty:
    email_cliente = row.iloc[0].get("Email","")

    if pd.isna(email_cliente):
        email_cliente = ""
    else:
        email_cliente = str(email_cliente).strip()

print("📧 EMAIL TROVATA:", email_cliente)
...
for lotto, colli_totali in lotti_raggruppati.items():
    ...
    # 🔥 QUI LA MAGIA
    quantita = colli_totali * 180

    descrizione = f"Uova sfuse - Lotto {lotto_fmt} - Scad {scadenza}"
...
def invia_email(destinatario, pdf_path, cliente, numero):
    ...
    msg = EmailMessage()
    msg["Subject"] = f"DDT {numero} - UOVA SFUSE"
    msg["From"] = mittente
    msg["To"] = destinatario
...
template = env.get_template("ddt_uova_sfuse.html")
...
vettore = f"{data.get('trasportatore','')} - Targa {data.get('targa','')}"
...
# 🔥 QUI CAMBIA
cartella_anno = os.path.join(base_archivio, f"{anno}_UOVA_SFUSE")
...
html_path = os.path.join(dest_dir, f"temp_uova_sfuse{num_str}.html")
...
pdf_path = os.path.join(
    dest_dir,
    f"DDT_UOVA_SFUSE{num_str}_{cliente_clean}.pdf"
)
print("📄 FILE:", pdf_path)
pdfkit.from_file(
    html_path,
